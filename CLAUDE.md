...
- The search tool queries an Azure AI Search index (default `bod-emails`: dojo emails, board minutes, bylaws, member records, calendar, web content) with hybrid retrieval — keyword + vector (`VectorizableTextQuery` against the index's `text_vector` field, embedded server-side by the index's integrated vectorizer) fused via RRF, then semantically reranked — with a pure-keyword fallback if the vectorizer or reranker fails. Claude can scope queries via optional tool params: `source_type` (email/calendar/member/web), `date_from`/`date_to` (mapped to `document_date` OData filters), and `newest_first` (sorts by `document_date desc`; runs in simple mode since `$orderby` is unsupported with semantic ranking). `SearchClient` is sync, so calls run via `run_in_executor`.
